...
class ProxyFetchSpider(Spider):
    ...
    def checkin(self, response):
        ...
        if 'startstring' in response.meta and (response.meta['startstring'] in res):
            proxy = response.meta['proxy']
            self.redis_db.sadd(self.PROXY_SET, proxy)
            logger.info(u'可用代理+1  %s' % proxy)
            yield None
    ...
    def parse_66ip(self, response):
        ...
        if 'proxy' in response.meta:
            logger.info(u'=>使用代理%s' % response.meta['proxy'])
        res = response.body_as_unicode()
    ...
    def closed(self, reason):
        logger.info(u'代理池更新完成，有效代理数: %s' % self.redis_db.scard(self.PROXY_SET))
